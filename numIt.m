...
function  [values] = numIt(modelFun,X, y, no_coefficients,...
...
coefficient_values_total = zeros(no_iterations*std_dev_iterations,...
                                 no_coefficients);
...
for f = 1:std_dev_iterations
    ...
    for i = 1:no_iterations
        ...
        for index = 1:no_coefficients
            random_bounds(index) = (ub(index)-lb(index)).*rand(1,1)...
                                    + lb(index);
        end
        
        problem = createOptimProblem('lsqcurvefit','x0',random_bounds,...
                                     'objective', modelFun,'lb',lb,...
                                     'ub',ub,'xdata',X,'ydata',y);
      
        ms = MultiStart;
        [xmulti,errormulti,flagm,outptm,manyminsm]=run(ms,...
                                                       problem,...
                                                       no_starting_points);
        ...
        elseif f ~= 1
            coefficient_values_total  = [coefficient_values_total;...
                                         coefficient_values];
        end

     for median_value = 1:no_coefficients
            median_values(f,median_value) = median(coefficient_values...
                                                   (:,median_value));       
     end
end
